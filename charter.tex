...
En la industria Química existen procesos continuos de fabricación los cuales deben ser controlados de forma permanente para asegurar una calidad constante entre lotes de un mismo producto. Para ello se implementan técnicas de mantenimiento predictivo intentando prevenir problemas que llevan a paros de emergencia o tiempos muertos en el proceso provocando una perdida parcial o total del lote concluyendo en un impacto financiero negativo.
Dado que en la mayoría de estos procesos se utilizan motores eléctricos como actuadores, se abre una gran posibilidad de desarrollar diversos tipos de sensores para aplicar mantenimiento predictivo. En la bibliografía se puede encontrar diversos métodos relacionados a evaluar el desgaste de un motor mediante el análisis de la corriente de consumo del mimo, es por ello que se plantea desarrollar un sensor de corriente capaz de medir, procesar y comparar datos estadísticos en dominio del tiempo y frecuencia.

La empresa Hitec SRl cuanta con un servicio de mantenimiento predictivo mediante la plataforma comercial \textit{teBox} de terative con análisis y reportes en la nube y se encuentra en la búsqueda de un sensor de corriente que se adapte a la plataforma. Por este motivo el presente proyecto busca desarrollar un sensor de corriente trifásico capaz de medir, procesar, almacenar y comunicar parámetros específicos a traves de dicha plataforma.

En la Fig \ref{fig:diagBloques} se presenta un diagrama en bloques del sensor de corriente trifásico que se propone desarrollar. se debe ADFADF
...
\begin{itemize}
\item Desplazamiento entre las 3 fases
...
\item ¿Ayuda a la explicación si se incluye un\\
...
\begin{itemize}
\item Todos los materiales necesarios para la implementación serán suministrados por Hictec SRL.
\item La importación de componentes no estará restringida y tomará un tiempo menor a 3 meses.
\item Se contará con asistencia por parte de Hitec SRL para el montaje de los componentes en el PCB.
\item Hitec SRL cuenta con todos los equipos para la medición y validación del prototipo.
\end{itemize}
...
\item Grupo de requerimientos asociados al sistema
	\begin{enumerate}
	\item La alimentación debe ser de 5V con un consumo máximo de 200 mA.
	\item 
	\item Requerimiento 3 (prioridad menor)
	\end{enumerate}
\item Grupo de requerimientos asociados con la comunicación
	\begin{enumerate}
	\item El protocolo está definido por la plataforma \textit{teBox}.
	\item El sensor se comporta como esclavo y la plataforma \textit{teBox} como maestro.
	\item Requerimiento 2 (prioridad menor)
	\end{enumerate}
\item Grupo de requerimientos asociados con las variables sensadas
	\begin{enumerate}
	\item El protocolo está definido por la plataforma \textit{teBox}.
	\item El sensor se comporta como esclavo y la plataforma \textit{teBox} como maestro.
	\item Requerimiento 2 (prioridad menor)
